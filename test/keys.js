...
describe('Test private key signatures and verify the proof', function() {
    it('Should sign all the private keys', function(done) {
        var keys = [
        ...
        var proof = index.signAll(keys, message, blockhash, currency, network);
        ...
        index.getBalance(addresses, function(err, balance) {
            assert(!err);
            assert(balance === 0);
            return done();
        });
