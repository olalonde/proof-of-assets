...
function signAll(type, keys, message, blockhash, currency, network) {
    if(!Array.isArray(keys))
        throw new Error('privateKeys must be an array');
    if (type === 'jbok') {
        return _signAllJbok(keys, message, blockhash, currency, network);
    } else if (type === 'bip32') {
        return _signAllBip32(keys, message, blockhash, currency, network);
    } else {
...
function _signAllBip32(xprivKeys, message, blockhash, currency, network) {
    var res = { type: 'bip32', message: message, blockhash: blockhash, currency: currency, network: network, signatures: [], addresses: [] };
...
function _verifyBip32(obj) {
    ...
    for (var i = 0; i < obj.addresses.length; i++) {
        ...
        var script = bitcoinjs.Script.createMultisigScriptPubKey(2, publicKeys);
        var hash = bitcoinjs.crypto.hash160(new Buffer(script.buffer));
        var address = new bitcoinjs.Address(hash, bitcoinjs.networks[obj.network].scriptHash).toString();
