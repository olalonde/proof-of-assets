...
function signAll(type, keys, message, blockhash, currency, network, requiredSignatures) {
    if(!Array.isArray(keys))
        throw new Error('privateKeys must be an array');
    if (type === 'jbok') {
        return _signAllJbok(keys, message, blockhash, currency, network);
    } else if (type === 'bip32') {
        return _signAllBip32(keys, message, blockhash, currency, network, requiredSignatures);
    } else {
...
function _signAllBip32(xprivKeys, message, blockhash, currency, network, requiredSignatures) {
    var res = { type: 'bip32', message: message, blockhash: blockhash, currency: currency, network: network, required_signatures: requiredSignatures, signatures: [], addresses: [] };
...
function _verifyBip32(obj) {
    ...
    for (var i = 0; i < obj.addresses.length; i++) {
        ...
        wallets.forEach(function(wallet) {
            publicKeys.push(wallet.derive(obj.addresses[i].index).pub);
        });
        if (wallets.length === 1) {
            // Simple derivation path
            address = wallets[0].derive(obj.addresses[i].index).getAddress(bitcoinjs.networks[obj.network].scriptHash).toString();
        ...
        if (address !== obj.addresses[i].address)
            return false;
    }
    return true;
};
...
function _getAddressesBip32(obj) {
    ...
    // remove duplicates. see http://stackoverflow.com/a/14740171/96855
    var dups = {};
    obj.addresses.forEach(function (hash) {
        if (dups[hash.address]) return;
        dups[hash.address] = true;
        addresses.push(hash.address);
    });
